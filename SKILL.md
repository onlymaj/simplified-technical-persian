...
This skill is not for creative or marketing copy — controlled Persian is deliberately flat and literal. Do not apply it to text where voice, nuance, or persuasion is the point.
...
No official "controlled Persian" standard exists — surveys of controlled natural languages list Easy Japanese and Français fondamental but no Persian entry. This skill therefore assembles its rules from the authorities that do exist, in priority order:
...
Where these authorities conflict (they occasionally do — e.g., هٔ vs ه‌ی for the ezafe after silent ه), this skill picks one form, says so, and applies it consistently. Consistency within a document outranks any single authority.
...
**Will not:**
- Claim Farhangestan certification. This skill follows دستور خطّ فارسی and اصول و ضوابط واژه‌گزینی as authorities, but official term approval is Farhangestan's alone — check فرهنگ واژه‌های مصوب for binding equivalents.
...
- **`references/research-report.md`** — the cited research behind every rule above: which claims were adversarially verified, which are single-sourced, and which were refuted.
